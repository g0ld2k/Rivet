...
Run: `swift test --filter CommitPromptTests`
Expected: PASS (5 tests). If the `@Generable` macro rejects the raw-value enum or the explicit init clashes with macro-generated members, consult the swiftinterface path in Global Constraints before changing shape.
...
```bash
swift build
cd "$(mktemp -d)" && git init -b main -q && echo 'let x = 1' > A.swift && git add A.swift
# from that directory, run the built binary:
<path-to-Rivet>/.build/debug/rivet commit-message
```

Expected: a Conventional Commit message on stdout (e.g. `feat: add A.swift constant`), rationale on stderr. Then verify the failure paths:

```bash
git reset -q                      # unstage
<path>/rivet commit-message; echo $?    # expect exit 4, "no staged changes" on stderr
cd /tmp && <path>/rivet commit-message; echo $?   # expect exit 3
```

And composability: `.../rivet commit-message 2>/dev/null | cat` prints only the message.
