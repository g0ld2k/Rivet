...
@Suite struct DiffSplitterTests {
    ...
    @Test func parsesChangedPostImagePathContainingBSlash() {
        let diff = """
        diff --git a/old.swift b/a b/c.swift
        @@ -1 +1 @@
        -old
        +new
        """
        ...
        #expect(DiffSplitter.pathFromHeader("diff --git a/old.swift b/a b/c.swift") == "a b/c.swift")
        #expect(sections.map(\.path) == ["a b/c.swift"])
    }

    @Test func parsesChangedPreImagePathContainingBSlash() {
        let diff = """
        diff --git a/a b/old.swift b/new.swift
        @@ -1 +1 @@
        -old
        +new
        """
        ...
        #expect(DiffSplitter.pathFromHeader("diff --git a/a b/old.swift b/new.swift") == "new.swift")
